...
Void FlxU_obj::openURL( ::String URL){
{
		...
		HX_STACK_LINE(22)
		::nme::Lib_obj::getURL(::neash::net::URLRequest_obj::__new(URL),HX_CSTRING("_blank"));
	}
return null();
}
...
int FlxU_obj::fromIntToUInt( int number){
	HX_STACK_PUSH("FlxU::fromIntToUInt","org/flixel/FlxU.hx",755);
	HX_STACK_ARG(number,"number");
	HX_STACK_LINE(755)
	return (  (((number < (int)0))) ? int(-(number)) : int(number) );
}


STATIC_HX_DEFINE_DYNAMIC_FUNC1(FlxU_obj,fromIntToUInt,return )
...
Dynamic FlxU_obj::__Field(const ::String &inName,bool inCallProp)
{
	...
	case 12:
		...
		break;
	case 13:
		if (HX_FIELD_EQ(inName,"fromIntToUInt") ) { return fromIntToUInt_dyn(); }
		break;
	case 14:
