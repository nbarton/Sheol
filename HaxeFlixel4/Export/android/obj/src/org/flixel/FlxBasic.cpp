	...
	HX_STACK_ARG(updateAtlas,"updateAtlas");
{
		...
		if (((this->_bitmapDataKey == null()))){
			HX_STACK_LINE(341)
			hx::Throw (HX_CSTRING("Something went wrong: _bitmapDataKey var isn't initialized"));
		}
		HX_STACK_LINE(346)
		if (((this->_atlas == null()))){
			HX_STACK_LINE(348)
				...
				if ((!(value->hasNodeWithName(this->_bitmapDataKey)))){
					...
					if (((bm == null()))){
						...
						if (((value->addNode(bm,this->_bitmapDataKey) == null()))){
							HX_STACK_LINE(311)
							hx::Throw (((HX_CSTRING("Can't add object's graphic to atlas: ") + value->name) + HX_CSTRING(". There isn't enough space")));
							HX_STACK_LINE(313)
							return null();
						}
					}
